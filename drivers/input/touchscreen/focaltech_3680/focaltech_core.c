...
static int fts_read_and_report_foddata(struct fts_ts_data *data)
{
	...
	case 0x25:
		if (data->nonui_status != 0) {
			FTS_INFO(
				"nonui_status is one/two, don't report key goto\n");
			return 0;
		}
	...
	case 0x26:
		...
		if (buf[8] == 0) {
			...
			if (data->suspended && data->fod_status == 0) {
				FTS_INFO(
					"Panel off and fod status : %d, don't report touch down event\n",
					data->fod_status);
			...
			if (!data->suspended) {
				...
				FTS_INFO(
					"FTS:touch is not in suspend state, report x,y value by touch nomal report\n");
				mutex_unlock(&data->report_mutex);
				return -EINVAL;
			}
			if (data->nonui_status == 2) {
				FTS_INFO(
					"nonui_status is two, don't report 152\n");
				mutex_unlock(&data->report_mutex);
				return 0;
			}
			...
			if (!data->fod_finger_skip && data->finger_in_fod) {
				...
				input_sync(data->input_dev);
				FTS_INFO(
					"Report_0x152 suspend DOWN report_area %d sucess for miui",
					data->overlap_area);
			}
			mutex_unlock(&data->report_mutex);
		} else {
	...
	case TOUCH_DEFAULT:
		...
		for (i = 0; i < max_touch_num; i++) {
			...
			if (ts_data->pdata->super_resolution_factors == 10) {
				events[i].x =
					((touch_buf[FTS_TOUCH_OFF_E_XH + base] &
					  0x0F)
					 << 11) +
					((touch_buf[FTS_TOUCH_OFF_XL + base] &
					  0xFF)
					 << 3) +
					(((touch_buf[FTS_TOUCH_OFF_PRE + base] &
					   0xC0) >>
					  6)
					 << 1) +
					((touch_buf[FTS_TOUCH_OFF_E_XH + base] &
				...
				events[i].y =
					((touch_buf[FTS_TOUCH_OFF_ID_YH + base] &
					  0x0F)
					 << 11) +
					((touch_buf[FTS_TOUCH_OFF_YL + base] &
					  0xFF)
					 << 3) +
					(((touch_buf[FTS_TOUCH_OFF_PRE + base] &
					   0x30) >>
				...
				events[i].area =
					touch_buf[FTS_TOUCH_OFF_AREA + base] &
					0x7F;
				events[i].p =
					touch_buf[FTS_TOUCH_OFF_PRE + base] &
					0x0F;
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
	...
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, pdata->x_min,
			     pdata->x_max - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MINOR, pdata->x_min,
			     pdata->x_max - 1, 0, 0);
#endif
...
static int fts_pinctrl_init(struct fts_ts_data *ts)
{
	...
	ts->pinctrl_state_spimode =
		pinctrl_lookup_state(ts->pinctrl, "pmx_ts_spi_mode");
	...
	ts->pinctrl_dvdd_enable =
		pinctrl_lookup_state(ts->pinctrl, "pmx_ts_dvdd_enable");
	...
	ts->pinctrl_dvdd_disable =
		pinctrl_lookup_state(ts->pinctrl, "pmx_ts_dvdd_disable");
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			if (ts_data->pinctrl && ts_data->pinctrl_dvdd_enable) {
				...
				if (ret)
					FTS_ERROR(
						"%s: Failed to enable dvdd, error= %d\n",
						__func__, ret);
				else
					FTS_ERROR(
		...
		if (!ts_data->power_disabled) {
			...
			if (ts_data->pinctrl && ts_data->pinctrl_dvdd_disable) {
				ret = pinctrl_select_state(
					ts_data->pinctrl,
					ts_data->pinctrl_dvdd_disable);
				if (ret) {
					FTS_ERROR(
						"%s: Failed to disable dvdd, error= %d\n",
						__func__, ret);
				} else
					FTS_ERROR(
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (pdata->have_key) {
		...
		FTS_INFO(
			"VK Number:%d, key:(%d,%d,%d), coords:(%d,%d),(%d,%d),(%d,%d)",
	...
	if (ret < 0) {
		FTS_ERROR(
			"Unable to get super-resolution-factors, please use default");
				     ...
				     bool enable)
{
	...
	FTS_INFO("AOD: %d DoubleClick: %d ",
		 ts_data->gesture_status >> 1 & 0x01,
		 ts_data->gesture_status & 0x01);
	FTS_INFO("gesture state:0x%02X, write cmd:0x%02X",
		 ts_data->gesture_status, ts_data->gesture_cmd);
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	if (ts_data->fts_tp_class == NULL) {
		...
		if (ts_data->fts_tp_class) {
			ts_data->fts_touch_dev =
				device_create(ts_data->fts_tp_class, NULL, 0x38,
					      ts_data, "tp_dev");
			...
			if (sysfs_create_file(&ts_data->fts_touch_dev->kobj,
...
static int fts_ts_suspend(struct device *dev)
{
	...
	FTS_INFO("%s : gesture_support:%d  poweroff_on_sleep:%d ", __func__,
		 ts_data->gesture_support, ts_data->poweroff_on_sleep);
...
static int fts_ts_resume(struct device *dev)
{
	...
	FTS_INFO("%s finger_in_fod:%d fod_finger_skip:%d\n", __func__,
		 ts_data->finger_in_fod, ts_data->fod_finger_skip);
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	(FTS_CHIP_TYPE == _FT8006P) || (FTS_CHIP_TYPE == _FT7120)
	spi->mode = SPI_MODE_1;
#else
	spi->mode = SPI_MODE_0;
#endif
