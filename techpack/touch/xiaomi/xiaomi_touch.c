				   ...
				   unsigned long arg)
{
	...
	if ((user_cmd == GET_CUR_VALUE) || (user_cmd == GET_DEF_VALUE) ||
		(user_cmd == GET_MIN_VALUE) || (user_cmd == GET_MAX_VALUE)) {
		if (touch_data->getModeValue)
			buf[0] = touch_data->getModeValue(buf[1], user_cmd);
	} else if (user_cmd == SET_CUR_VALUE) {
		...
		sysfs_notify(&xiaomi_touch_device->dev->kobj, NULL,
			     "touch_thp_cmd");
	} else if (user_cmd == GET_MODE_VALUE) {
		if (touch_data->getModeValue)
			ret = touch_data->getModeAll(buf[1], buf);
	} else if (user_cmd == SET_LONG_VALUE)
		pr_info("%s: Not supported, continue", __func__);
	else {
...
static ssize_t thp_dump_raw_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	struct xiaomi_touch_interface *touch_data = NULL;
...
static ssize_t thp_dump_raw_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	struct xiaomi_touch_interface *touch_data = NULL;
...
static DEVICE_ATTR(touch_thp_dump, (S_IRUGO | S_IWUSR | S_IWGRP), thp_dump_raw_show, thp_dump_raw_store);
...
static int xiaomi_touch_probe(struct platform_device *pdev)
{
	...
	pdata->raw_data = (unsigned int *)kzalloc(PAGE_SIZE * 12, GFP_KERNEL);
	if (!pdata->raw_data) {
		ret = -ENOMEM;
	...
	for (i = 0; i < RAW_BUF_NUM; i++) {
		pdata->raw_buf[i] =
			(unsigned int *)kzalloc(PAGE_SIZE * 12, GFP_KERNEL);
		if (!pdata->raw_buf[i]) {
			ret = -ENOMEM;
	...
	pr_debug("%s: kernel base:%lld", __func__,
		 (unsigned long)pdata->raw_data);
	pr_debug("%s: phy base:%lld\n", __func__,
...
module_platform_driver(xiaomi_touch_device_driver);
